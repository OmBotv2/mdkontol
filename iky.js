...
//const { match, stop, isMatched } = require('./lib/matchmaking')       
      ...
      //  const isGroupOwner = groupOwner.includes(sender) || false
        ...
        const io = ["6285349385702@s.whatsapp.net", "6282239202895@s.whatsapp.net"]//
...
const getRegisteredRandomId = () => {
    ...
    //REGISTER
    const maaf = `*MAAF KAK ${pushname}, IZUMI BELUM MENGENALI KAKAK*\n\n*KLIK TOMBOL "VERIFY" UNTUK DAFTAR>//<*`
...
*「 IZUMI BOTZ 」*`
    ...
    // buff = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
        ...
        // console.log(e)
            ...
            reply(`*Terimakasih Telah Terdaftar Di Izumi Botz *`)
        }, 2000)
        break */
          case 'verify':
...
`
              ...
              buttonsMessage = {footerText:'@ I Z U M I  B O T', imageMessage: imageMsg,
...
*Selamat datang di menu IZUMI BOT*
*Izumi akan membantu apa yang di butuhkan kakak seperti membuat sticker dan lain-lain.*
────────────────────
...
let ayang = fs.readFileSync('./media/kiki.jpg')
               buttons =  [{buttonId: `${prefix}allmenu`, buttonText: {displayText: '📜ALL MENU'}, type: 1},{buttonId: `${prefix}rules`, buttonText: {displayText: '✖RULES'}, type: 1},{buttonId: `${prefix}owner`, buttonText: {displayText: '👥CREATOR'}, type: 1},]
                    ...
                    dimas.sendMessage(from, { contentText: menu, footerText: '_*Jika kalian pakai wa mod,bisa mengetik !command*_\n\n@', buttons: buttons, headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ayang, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage', { quoted: dim})
   break
    case 'allmenu':
              if (!isRegistered) return sendButMessage(from, maaf, mod, tombar,{quoted:dim})
       groups = dimas.chats.array.filter(v => v.jid.endsWith('g.us'))
...
*Selamat datang di menu IZUMI-Bots BOT*
*By:Alwi Ofc*
...
┗━▹
`
                   sendButMessage(from, wibu, `@ D I M A S\n@ I Z U M I  B O T`, [
          {
            buttonId: `!menu`,
            buttonText: {
              displayText: `⬡ BACK TO MENU `,
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `NEXT`},type:1}]
              imageMsg = (await dimas.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Crated by Alwi Gans', imageMessage: imageMsg,
...
`
 ...
 ♥️ SONGKO RACCA\n\n♥️ Alwi ofc`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
\`\`\`▢ Link : ${i.url}\`\`\``
              ...
              contentText:`Follow @`,buttons,headerType:4}
              prep = await dimas.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              dimas.relayWAMessage(prep)
              break
              case 'quotemaker':
...
case 'ban':
 ...
 case 'blockk':
      ...
      case 'broadcast':
             ...
             for (let _ of anu) {
             dimas.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 IZUMI BROADCAST 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
             sendMess(_.jid, `*「 IZUMI BROADCAST 」*\n\n${body.slice(4)}`, {quoted:freply})
}
             reply('Suksess broadcast')
}
             break
...
dimas.sendMessage(_.jid, 
   ...
   "buttonText": {"displayText": "Join Group"
             ...
             bc2 = await dimas.downloadMediaMessage(encmedia) 
             for (let _ of anu) { 
             dimas.sendMessage(_.jid, { contentText: `*🌹 SIARAN IZUMI BOTZ 🌹*\n\n${body.slice(4)}`, footerText: '©Created by ', buttons: [{ buttonId: `${prefix}menu`, buttonText: { displayText: 'MENU' }, type: 1 }, { buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER' }, type: 1 }], headerType: 6, locationMessage: { degreesLatitude: 0, degreesLongitude: 0, jpegThumbnail: bc2 }}, 'buttonsMessage') 
} 
             reply('Suksess broadcast') 
} 
             break
